...
def get_all_proposal_ids(ids_filename):
    ...
    while True:
        response = send_request('ris_antrag_trefferliste', 'nav=1', {
                                'txtPosition': txtpos, 'txtVon': '01.01.2001'})
        new_ids = re.findall(
            'ris_antrag_detail\.jsp\?risid=([0-9]+)', response, re.S)
...
def get_all_proposals(ids_filename, proposals_filename):
    ...
    while True:
        ...
        json.dump(results, result_file, sort_keys=True,
...
def get_proposal_details(id):
    ...
    details_left = re.findall(
        '<div class="detail_div_left">(.*?)</div>', response, re.S)
    details_right = re.findall(
        '<div class="detail_div_right">(.*?)</div>', response, re.S)
    details_left_long = re.findall(
        '<div class="detail_div_left_long">(.*?)</div>', response, re.S)
...
def get_proposal_documents(id):
    ...
    document_details = re.findall(
        'href="/RII/RII/DOK/ANTRAG/([0-9]*?)\.pdf">(.*?)</a>', response, re.S)
    documents = []
...
def get_proposal_results(id):
    ...
    for row in rows:
        ...
        if cols[4] == '&nbsp;':  # skip results which have an empty description
            continue
        gremium = re.findall(
            'ris_gremien_detail.jsp\?risid=([0-9]+?)&periodeid=([0-9]+?)">(.*?)</a>', cols[0], re.S)
        if len(gremium) > 0:  # sometimes there are ba_gremien instead of ris_gremien
            gremium = gremium[0]
        else:
            gremium = re.findall(
                'ba_gremien_details.jsp\?Id=([0-9]+?)&Wahlperiode=([0-9]+?)" target="_blank">(.*?)</a>', cols[0], re.S)[0]
        setting_date = re.findall(
            'ris_sitzung_detail.jsp\?risid=([0-9]+)">([0-9]{2}\.[0-9]{2}\.[0-9]{4}), ([0-9]{2}:[0-9]{2})</a>', cols[1], re.S)
        if len(setting_date) > 0:  # sometimes there are ba_sitzungen instead of ris_sitzungen
            setting_date = setting_date[0]
        else:
            setting_date = re.findall(
                'ba_sitzungen_details.jsp\?Id=([0-9]+)" target="_blank">([0-9]{2}\.[0-9]{2}\.[0-9]{4}), ([0-9]{2}:[0-9]{2})</a>', cols[1], re.S)[0]
        template = re.findall(
            'ris_vorlagen_detail.jsp\?risid=([0-9]+)">(.*?)</a>', cols[3], re.S)[0]
        resolution_file_prev = resolution_file = re.findall(
            'href="/RII/RII/DOK/TOP/([0-9]+)\.pdf"', cols[3], re.S)
        if len(resolution_file_prev) > 0:
            resolution_file = resolution_file_prev[0]
